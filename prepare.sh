...
if [ "$EXTRA_DNF_PACKAGES" ]; then
    echo "EXTRA_DNF_PACKAGES environment variable found.  Installing."
    dnf upgrade -y
    dnf install -y $EXTRA_YUM_PACKAGES
fi
...
dnf remove python38-pip-wheel
dnf remove python38-setuptools-wheel
dnf remove python38-pip
dnf remove python38-setuptools
